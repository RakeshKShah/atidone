...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="missing_body_field_rejected"
USER_ID="user-42-${CASE_SUFFIX}"
TODO_ID="todo-777-${CASE_SUFFIX}"
...
printf '{}' > "$PATCH_BODY_FILE"

echo "STEP: Given — create authenticated session and owned todo for missing field validation"
...
VALUES ('${TODO_ID}', '${USER_ID}', 'Missing field target ${CASE_SUFFIX}', 0, NOW())
ON CONFLICT (id) DO NOTHING;
SQL

# When — perform the action under test
echo "STEP: When — PATCH /api/todos/{id} with an empty body"
...
echo "STEP: Then — verify missing required completed field is rejected"
[ "$status" = "400" ] || { echo "ASSERTION_FAILED: expected HTTP 400 got ${status}"; exit 1; }
grep -Eiq 'completed|required|body|validation' "$BODY_FILE" || { echo "ASSERTION_FAILED: expected validation error mentioning required completed field"; exit 1; }
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -t -A -F ',' -c "SELECT id, user_id, completed FROM todos WHERE id = '${TODO_ID}';" > "$DB_FILE"
grep -F "${TODO_ID},${USER_ID},0" "$DB_FILE" || { echo "ASSERTION_FAILED: expected todo to remain unchanged after missing field request"; exit 1; }
...
echo "CODEVALID_TEST_ASSERTION_OK:missing_body_field_rejected"
