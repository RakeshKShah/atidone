...
echo "STEP: Given — create authenticated user for invalid id request"
echo "PREREQ: inserting user row directly in database"
...
echo "STEP: When — delete todo with invalid id format"
...
echo "STEP: Then — validation rejects malformed id before deletion"
[ "$code" = "400" ] || { echo "ASSERTION_FAILED: expected HTTP 400 got ${code}"; exit 1; }
if command -v jq >/dev/null 2>&1; then
  jq -e '(.statusCode == 400) or (.status == 400)' "$WHEN_BODY" >/dev/null || { echo "ASSERTION_FAILED: expected validation error payload with 400 status"; exit 1; }
else
  grep -F '400' "$WHEN_BODY" >/dev/null || { echo "ASSERTION_FAILED: expected response body to mention 400"; exit 1; }
fi
