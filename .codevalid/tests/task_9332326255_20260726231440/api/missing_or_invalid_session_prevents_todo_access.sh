...
echo "STEP: When — request todo API without any session cookie"
...
echo "STEP: When — request todo API with an invalid session cookie"
echo "REQUEST_HEADERS: Accept: application/json"
echo "REQUEST_BODY:"
...
case "$status_empty" in
  302|303|307|308|401) ;;
  *)
    echo "ASSERTION_FAILED: expected missing session request to return 3xx or 401 got ${status_empty}"
    exit 1
    ;;
esac
case "$status_invalid" in
  302|303|307|308|401) ;;
  *)
    echo "ASSERTION_FAILED: expected invalid session request to return 3xx or 401 got ${status_invalid}"
    exit 1
    ;;
esac
if [ "$status_empty" = "200" ] || [ "$status_invalid" = "200" ]; then
  echo "ASSERTION_FAILED: invalid or missing session unexpectedly returned HTTP 200"
  exit 1
fi
