...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="authenticated_update_other_user_todo_forbidden"
...
cleanup_files() {
  rm -f "$OWNER_COOKIE_JAR" "$ATTACKER_COOKIE_JAR" "$CREATE_HEADERS" "$CREATE_BODY" "$PATCH_HEADERS" "$PATCH_BODY" "$OWNER_LIST_HEADERS" "$OWNER_LIST_BODY" "$CREATE_REQ_BODY" "$PATCH_REQ_BODY" "$CLEANUP_HEADERS" "$CLEANUP_BODY"
}
...
echo "STEP: Given — create separate authenticated owner and attacker sessions"
...
echo "PREREQ: bootstrap attacker session"
AUTH_TEST_USER="attacker-${CASE_SUFFIX}" ./tests/task_9332326255_20260726231440/api/_auth_session_helper.sh "$TEST_ID-attacker" "$CASE_SUFFIX" "$ATTACKER_COOKIE_JAR"
...
echo "STEP: When — attacker PATCHes the owner's todo id"
echo "REQUEST_HEADERS:"
echo 'Content-Type: application/json'
...
echo "STEP: Then — verify cross-user update is rejected and owner data is unchanged"
...
jq -e --arg id "$TODO_ID" 'map(select(.id == $id and (.completed == false or .completed == 0))) | length == 1' "$OWNER_LIST_BODY" >/dev/null || { echo "ASSERTION_FAILED: expected owner todo ${TODO_ID} to remain incomplete"; exit 1; }
...
echo "CODEVALID_TEST_ASSERTION_OK:authenticated_update_other_user_todo_forbidden"
