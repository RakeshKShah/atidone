...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="create_todo_response_structure"
COOKIE_FILE="${COOKIE_FILE:-${AUTH_COOKIE_FILE:-}}"
TITLE="Test todo for response validation"
...
SHORT_GIVEN="authenticated session for structure test user is available"
...
SHORT_WHEN="create todo and inspect response structure"
...
SHORT_THEN="response contains expected fields and values"
echo "STEP: Then — ${SHORT_THEN}"
[ "$code" = "200" ] || [ "$code" = "201" ] || { echo "ASSERTION_FAILED: expected HTTP 200 or 201 got ${code}"; exit 1; }
if command -v jq >/dev/null 2>&1; then
  jq -e 'type == "object"' "$BODY_FILE" >/dev/null || { echo "ASSERTION_FAILED: expected JSON object response"; exit 1; }
  CREATED_TODO_ID="$(jq -r '.id' "$BODY_FILE")"
  [ "$CREATED_TODO_ID" != "null" ] && [ -n "$CREATED_TODO_ID" ] || { echo "ASSERTION_FAILED: expected id field"; exit 1; }
  USER_ID="$(jq -r '.userId' "$BODY_FILE")"
  [ "$USER_ID" = "user-structure-test" ] || { echo "ASSERTION_FAILED: expected userId user-structure-test got '$USER_ID'"; exit 1; }
  ...
  [ "$CREATED_AT" != "null" ] && [ -n "$CREATED_AT" ] || { echo "ASSERTION_FAILED: expected createdAt field"; exit 1; }
else
  grep -F '"userId":"user-structure-test"' "$BODY_FILE" >/dev/null || { echo "ASSERTION_FAILED: expected response body userId user-structure-test"; exit 1; }
  grep -F '"title":"Test todo for response validation"' "$BODY_FILE" >/dev/null || { echo "ASSERTION_FAILED: expected response body title"; exit 1; }
  grep -F '"createdAt":' "$BODY_FILE" >/dev/null || { echo "ASSERTION_FAILED: expected response body createdAt"; exit 1; }
...
SHORT_CLEANUP="delete created todo after structure validation"
...
echo "CODEVALID_TEST_ASSERTION_OK:create_todo_response_structure"
