...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="create_todo_missing_title_validation_error"
...
SHORT_WHEN="submit POST /api/todos without title"
...
SHORT_THEN="schema validation rejects missing title"
echo "STEP: Then — ${SHORT_THEN}"
[ "$code" = "400" ] || [ "$code" = "422" ] || { echo "ASSERTION_FAILED: expected HTTP 400 or 422 got ${code}"; exit 1; }
grep -Ei 'title|required|invalid|validation' "$BODY_FILE" >/dev/null || { echo "ASSERTION_FAILED: expected validation message mentioning title or required field"; exit 1; }

# Cleanup — undo Given side effects
SHORT_CLEANUP="no cleanup required because creation failed"
echo "STEP: Cleanup — ${SHORT_CLEANUP}"

echo "CODEVALID_TEST_ASSERTION_OK:create_todo_missing_title_validation_error"
