...
echo "STEP: Given — bootstrap authenticated session and create the user's todo via HTTP"
...
echo "STEP: When — perform PATCH on the authenticated user's own todo"
echo "REQUEST_HEADERS: Content-Type: application/json"
echo "REQUEST_HEADERS: Cookie jar ${COOKIE_JAR}"
echo "REQUEST_BODY:"
...
echo "STEP: Then — assert 200 response and persisted completed=true in the user's todo list"
...
grep -F '"completed":true' "$PATCH_BODY" >/dev/null || grep -F '"completed":1' "$PATCH_BODY" >/dev/null || { echo "ASSERTION_FAILED: expected response body to show completed=true"; exit 1; }
...
grep -F '"title":"'"${TODO_TITLE}"'"' "$LIST_BODY" >/dev/null || { echo "ASSERTION_FAILED: expected list response to contain todo title ${TODO_TITLE}"; exit 1; }
grep -F '"completed":true' "$LIST_BODY" >/dev/null || grep -F '"completed":1' "$LIST_BODY" >/dev/null || { echo "ASSERTION_FAILED: expected persisted todo to remain completed=true"; exit 1; }
