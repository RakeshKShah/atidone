...
CASE_SUFFIX="$(date +%s)-$$"
TEST_ID="authenticated_user_updates_own_todo_to_completed"
USER_ID="user-123"
...
echo "STEP: Given — create authenticated user's todo fixture"
echo "PREREQ: inserting todo ${TODO_ID} for user ${USER_ID} with completed=0"
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 <<SQL
DELETE FROM todos WHERE id = '${TODO_ID}';
INSERT INTO todos (id, user_id, title, completed) VALUES ('${TODO_ID}', '${USER_ID}', 'Seed todo ${CASE_SUFFIX}', 0);
SQL
...
echo "STEP: When — PATCH authenticated user's own todo to completed=true"
...
echo "STEP: Then — assert 200 response and persisted completed=1"
...
grep -F '"completed":1' "$PATCH_BODY" >/dev/null || grep -F '"completed":true' "$PATCH_BODY" >/dev/null || { echo "ASSERTION_FAILED: expected response body to show completed updated"; exit 1; }
grep -F '"userId":"'"${USER_ID}"'"' "$PATCH_BODY" >/dev/null || grep -F '"user_id":"'"${USER_ID}"'"' "$PATCH_BODY" >/dev/null || { echo "ASSERTION_FAILED: expected response body to retain user id ${USER_ID}"; exit 1; }
...
printf '%s' "$DB_ROW" | grep -F "${TODO_ID}|${USER_ID}|1" >/dev/null || { echo "ASSERTION_FAILED: expected database row ${TODO_ID}|${USER_ID}|1 but got ${DB_ROW}"; exit 1; }

echo "STEP: Cleanup — delete seeded todo row"
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 -c "DELETE FROM todos WHERE id = '${TODO_ID}';"

echo "CODEVALID_TEST_ASSERTION_OK:authenticated_user_updates_own_todo_to_completed"
